...
public:
  ...
  PatchWorkpp(double sensor_height, int num_iter, int num_lpr, int num_min_pts,
              double th_seeds, double th_dist, double th_seeds_v,
              double th_dist_v, double max_range, double min_range,
              double uprightness_thr, double adaptive_seed_selection_margin,
              double RNR_ver_angle_thr, double RNR_intensity_thr,
              int max_flatness_storage, int max_elevation_storage,
              bool enable_RNR, bool enable_RVPF, bool enable_TGR, int num_zones,
      ...
      : sensor_height_(sensor_height), num_iter_(num_iter), num_lpr_(num_lpr),
        num_min_pts_(num_min_pts), th_seeds_(th_seeds), th_dist_(th_dist),
        th_seeds_v_(th_seeds_v), th_dist_v_(th_dist_v), max_range_(max_range),
        min_range_(min_range), uprightness_thr_(uprightness_thr),
        adaptive_seed_selection_margin_(adaptive_seed_selection_margin),
        RNR_ver_angle_thr_(RNR_ver_angle_thr),
        RNR_intensity_thr_(RNR_intensity_thr),
        max_flatness_storage_(max_flatness_storage),
        max_elevation_storage_(max_elevation_storage), enable_RNR_(enable_RNR),
        enable_RVPF_(enable_RVPF), enable_TGR_(enable_TGR),
        num_zones_(num_zones), num_sectors_each_zone_(num_sectors_each_zone),
        num_rings_each_zone_(num_rings_each_zone),
        elevation_thr_(elevation_thr), flatness_thr_(flatness_thr) {
    ...
    //        "reject_pc", 100, true);
    //    pub_normal =
    //        node_handle_.advertise<sensor_msgs::PointCloud2>("normals", 100,
    //        true);
    //    pub_noise =
    //        node_handle_.advertise<sensor_msgs::PointCloud2>("noise", 100,
    //        true);
    //    pub_vertical =
    //        node_handle_.advertise<sensor_msgs::PointCloud2>("vertical", 100,
    //        true);
...
private:
  ...
  jsk_recognition_msgs::msg::PolygonArray poly_list_;

  //  ros::Publisher PlaneViz, pub_revert_pc, pub_reject_pc, pub_normal,
  //  pub_noise,
  //      pub_vertical;
   ...
   */
  geometry_msgs::msg::PolygonStamped set_polygons(int zone_idx, int r_idx,
...
/*
    @brief Velodyne pointcloud callback function. The main GPF pipeline is
    here. PointCloud SensorMsg -> Pointcloud -> z-value sorted Pointcloud
    ->error points removal -> extract ground seeds -> ground plane fit
    mainloop
*/
  ...
  for (int zone_idx = 0; zone_idx < num_zones_; ++zone_idx) {
    ...
    for (int ring_idx = 0; ring_idx < num_rings_each_zone_[zone_idx];
      ...
      for (int sector_idx = 0; sector_idx < num_sectors_each_zone_[zone_idx];
        ...
        /*
            About 'is_heading_outside' condidition, heading should be
            smaller than 0 theoretically. ( Imagine the geometric
            relationship between the surface normal vector on the
            ground plane and
                the vector connecting the sensor origin and the mean
                point of the ground plane )

            However, when the patch is far awaw from the sensor
            origin, heading could be larger than 0 even if it's ground
            due to lack of amount of ground plane points.
        ...
        */
        ...
        /*
            ...
            and TGR (Temporal Ground Revert). More information in the
            paper Patchwork++.
        */
...
inline void PatchWorkpp<PointT>::temporal_ground_revert(
  ...
  for (size_t i = 0; i < candidates.size(); i++) {
    ...
    // Debug
    if (verbose_) {
      cout << "\033[1;33m" << candidate.sector_idx
           << "th flat_sector_candidate "
    ...
    if (candidate.line_variable >
        8.0) //&& candidate.line_dir > M_PI/4)
             // candidate.ground_elevation > elevation_thr_[concentric_idx])
    {
...
template <typename PointT>
inline geometry_msgs::msg::PolygonStamped
...
template <typename PointT>
inline void
PatchWorkpp<PointT>::pc2czm(const pcl::PointCloud<PointT> &src,
                            std::vector<Zone> &czm,
                            pcl::PointCloud<PointT> &cloud_nonground) {
